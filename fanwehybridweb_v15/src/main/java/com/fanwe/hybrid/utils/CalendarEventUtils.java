...
public class CalendarEventUtils {
    ...
    private static final String ACCOUNT_NAME = "日历";
    ...
    public static void insertEvent(EventModel model) {
        ...
        @SuppressLint("MissingPermission") Uri newEvent = App.sContext.getContentResolver().insert(eventsUri, event);
        ...
        App.sContext.getContentResolver().insert(remindersUri, values);
    }
    ...
    public static List<EventModel> queryEvents() {
        ...
        // 如果是谷歌的账户是可以同步到服务器的
        cursor = App.sContext.getContentResolver().query(eventsUri, EVENTS_COLUMNS,
    ...
    public static void updateEvent(EventModel model) {
        ...
        App.sContext.getContentResolver().update(eventsUri, contentValues
    ...
    public static int deleteEvent(String id) {
        return App.sContext.getContentResolver()
    ...
    private static String queryCalId() {
        ...
        try {
            userCursor = App.sContext.getContentResolver().query(calendarsUri, null, "name=?", new String[]{ACCOUNT_NAME}, null);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (userCursor == null) {
    ...
    private static void addAccount() {
        ...
        App.sContext.getContentResolver().insert(calendarUri, value);
    }
